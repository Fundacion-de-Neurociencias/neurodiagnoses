...
with gr.Blocks(theme=gr.themes.Soft(), title="Neurodiagnoses") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("Clinical: Single-Patient Report"):
            ...
            subject_id_input = gr.Textbox(label="Subject ID (Mandatory)", value="NACC_11782")
            age_input = gr.Number(label="Age (Mandatory)", value=74)
            sex_input = gr.Radio(label="Sex", choices=["Male", "Female"], value="Female")
            
            run_single_button = gr.Button("Generate Tridimensional Annotation", variant="primary")
            
            gr.Markdown("---")
            gr.Markdown("### Final Diagnostic Annotation")
            single_output_report = gr.Textbox(label="Annotation", lines=5, interactive=False)
        ...
        with gr.TabItem("Research: Cohort Analysis"):
            ...
            cohort_output_table = gr.DataFrame(label="Tridimensional Annotations for Cohort", wrap=True)
