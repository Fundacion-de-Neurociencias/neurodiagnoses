...
def test_biomarker_creation():
    ...
    assert (
        "Biomarker(name='MMSE', value=28, unit='points', category=<BiomarkerCategory.CLINICAL_PHENOTYPIC: 'Clinical Phenotypic Markers'>)"
        in repr(b)
    )
...
def test_add_biomarker_to_patient():
    ...
    p.add_biomarker(
        name="pTau", value=35.2, unit="pg/mL", category=BiomarkerCategory.FLUID
    )
...
def test_get_biomarker_from_patient():
    ...
    p.add_biomarker(
        name="NfL", value=1200, unit="pg/mL", category=BiomarkerCategory.FLUID
    )
