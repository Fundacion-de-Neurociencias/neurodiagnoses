...
def run_validation():
    ...
    """
    print("--- Starting Scientific Validation Pipeline for Axis 2 ---")
    ...
    try:
        ...
        if "biomarkers_Sex_value" not in df.columns:
            df["biomarkers_Sex_value"] = np.random.choice(
                ["Male", "Female"], df.shape[0]
            )
        # -----------------

    except Exception as e:
        print(
            f"ERROR: Could not read or process the dataset at {DATA_PATH}. Error: {e}"
        )
        return
    ...
    for fold, (train_index, test_index) in enumerate(skf.split(X, y)):
        ...
        model = (
            fold_pipeline.model
            if fold_pipeline.model
            else (
                joblib.load(fold_pipeline.model_path)
                if os.path.exists(fold_pipeline.model_path)
                else fold_pipeline.train_and_evaluate()
            )
        )
        ...
        all_metrics.append(
            {"fold": fold + 1, "auc": auc, "accuracy": accuracy, "f1_weighted": f1}
        )
        print(
            f"  Fold {fold + 1}/{N_SPLITS} | AUC: {auc:.4f} | Accuracy: {accuracy:.4f}"
        )
    ...
    for sex, group in sex_groups:
        ...
        f1_sex = f1_score(
            group[pipeline.target], group["predicted_class"], average="weighted"
        )
