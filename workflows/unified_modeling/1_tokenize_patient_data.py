...
class PatientDataTokenizer:
    ...
    def build_vocabulary(self, df):
        ...
        feature_names = [
            col for col in df.columns if col not in ["patient_id", "visit_date"]
        ]
    ...
    def run_tokenization(self):
        ...
        first_patient_df = df[df["patient_id"] == first_patient_id].sort_values(
            by="biomarkers_Age_value"
        )
        ...
        human_readable = [
            self.reverse_vocabulary.get(t, str(t)) for t in tokenized_sequence[:30]
        ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Tokenize patient data for unified modeling."
    )
    parser.add_argument(
        "--data_path",
        type=str,
    ...
    tokenizer = PatientDataTokenizer(
