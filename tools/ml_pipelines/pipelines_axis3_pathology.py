...
class Axis3PathologyPipeline:
    def __init__(self, data_path="data/cornblath/pathology_matrix.csv"):
        ...
        print(
            f"INFO: Axis 3 Pathology Pipeline initialized with data from '{data_path}'."
        )
    ...
    def predict(self, patient_id):
        ...
        if not tau_columns:
            return (
                "Phenotype (Pathology) could not be determined: No tau columns found."
            )

        top_tau_regions = patient_scores[tau_columns].nlargest(3)

        region_strings = [
            f"{region.replace('_tau', '')} (score: {score:.2f})"
            for region, score in top_tau_regions.items()
        ]
        phenotype_description = (
            "Neuropathology Profile (t-Tau): High burden in "
            + ", ".join(region_strings)
        )
