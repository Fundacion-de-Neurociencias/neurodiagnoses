...
def parse_cornblath_patient(patient_id, metadata, data_dir='data/cornblath'):
    ...
    """
    try:
        pathology_df = pd.read_csv(os.path.join(data_dir, 'cornblath/pathology_matrix.csv'))
    ...
    if not pathology_data.empty:
        record.neuropathology_data = pathology_data.drop(columns=['projid']).to_dict('records')[0]
