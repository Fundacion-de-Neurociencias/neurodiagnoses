...
def merge_records(
    base_record: PatientRecord, new_record: PatientRecord
) -> PatientRecord:
    ...
    if new_record.genetics:
        ...
        if not base_record.genetics:
            ...
            base_record.genetics.disease_specific_risk.extend(
                new_record.genetics.disease_specific_risk
            )
    return base_record


def ingest_generic_patient(
    patient_id,
    clinical_csv,
    imaging_csv,
    genomics_csv,
    output_dir="patient_database/generic",
):
    """
...
if __name__ == "__main__":
    ...
    if not os.path.exists(sample_clinical_path):
        pd.DataFrame([{"patient_id": "ND_001", "age": 75, "MMSE": 25}]).to_csv(
            sample_clinical_path, index=False
        )
    if not os.path.exists(sample_imaging_path):
        pd.DataFrame(
            [{"patient_id": "ND_001", "hippocampal_volume_norm": 4500.0}]
        ).to_csv(sample_imaging_path, index=False)
    if not os.path.exists(sample_genomics_path):
        pd.DataFrame([{"patient_id": "ND_001", "APOE4_alleles": 1}]).to_csv(
            sample_genomics_path, index=False
        )
